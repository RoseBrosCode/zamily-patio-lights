...
class PyoClient:
        ...
        if duplex:
            if audio_backend != "portaudio":
                ...
                if input_device_id and output_device_id:
                    ...
                    logging.debug("Audio input device %s connected." % input_devices[input_device_id]['name'])
                    logging.debug("Audio output device %s connected." % output_devices[output_device_id]['name'])
                else:
                    if not prompt_for_devices:  # attempt to set up default device
                        ...
                        if not (default_input_found and default_output_found):
                            logging.debug("Unable to attach to default audio device, prompting for selection.")
    ...
    def setup_midi(self, prompt_for_devices=True):
        ...
        try:
            ...
            logging.debug("Unable to setup MIDI.", e)
